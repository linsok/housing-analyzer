...
facilities: property.facilities ? 
              (typeof property.facilities === 'string' ? 
                (property.facilities.startsWith('[') ? 
                  ...
                  property.facilities.split(',').map(f => f.trim()).filter(f => f)) : 
                property.facilities) : [],
          ...
          setPropertyData(property); // Store full property data
          ...
          if (error.message.includes('JSON')) {
            toast.error('Error parsing property data. Please try again.');
          } else {
            const errorMessage = error.response?.data?.message || 'Failed to load property. Please try again.';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editMode) {
        ...
        if (images.length > 0) {
          await propertyService.uploadImages(response.id, images);
        }
          ...
          <p className="text-gray-600 mt-2">
            {editMode ? (
              propertyData?.verification_status === 'verified' ? (
                'Editing this property will resubmit it for admin review.'
              ) : (
                'Update your property details and resubmit for admin review.'
              )
            ) : (
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex gap-4">
            ...
            <Button
              ...
              className="flex-1"
            >
              {loading ? 'Submitting...' : (editMode ? 'Resubmit Property for Review' : 'Submit Property for Review')}
            </Button>
          </div>
        </form>
      </div>
